...
class NeuronMLPObjective:
    ...
    def __init__(self, epochs, train_loader, val_loader, test_loader) -> None:
        ...
        self.epochs = epochs
        self.device = None
        self.neron_path = os.path.dirname(os.path.abspath(__file__))
    ...
    def train(self):
        ...
        self._save_neuron_model(self.model)
        return epoch_losses

    def _save_neuron_model(self, model, save_path=None):
        if not save_path:
            save_path = self.neuron_path
        model.eval()
        neuron_model = torch_neuron.trace(
            model, example_inputs=[self.val_laoder], dynamic_batch_size=True)
