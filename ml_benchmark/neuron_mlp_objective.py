...
class NeuronMLPObjective:
    ...
    def validate(self):
        self.model.eval()
        self.model = torch_neuron.trace(
            self.model, example_inputs=[self.val_laoder], dynamic_batch_size=True)
        val_targets = []
        val_preds = []
    ...
    def test(self):
        self.model.eval()
        self.model = torch_neuron.trace(
            self.model, example_inputs=[self.test_laoder], dynamic_batch_size=True)
        test_targets = []
        test_predictions = []
